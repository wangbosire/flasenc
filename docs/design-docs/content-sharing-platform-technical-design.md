...
- **权益与码**：**`POST .../content-entitlements`**、**`POST .../redemption-codes`**；`createdByUserId` 与审计同事务。  
- **内容处置与复核**：**`unlist` / `hide` / `restore-listing`**；**`GET .../contents/{id}`** 平台只读；**`GET .../contents/queues/suspicious`** 与 **`clear-suspicion` / `mark-manually-rejected`**。  
