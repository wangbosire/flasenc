...
- **环境**：API 基址、环境标识等来自 **环境变量**（如 `import.meta.env` / Rsbuild `process.env`），禁止把生产地址写死在业务代码中。
...
- **React 19**、**Rsbuild**、**Tailwind CSS v4**、**Biome**、**ESLint**、**rstest**、**TypeScript**。

### 2.2 目录与职责（推荐演进方向）

| 区域 | 约定 |
|------|------|
| `src/` | 应用源码根；随规模增大可按 **功能域** 分子目录（如 `features/`、`components/`、`lib/`、`api/`），新增时在同应用 README 或本文补充一行说明。 |
| `src/api/`、`src/lib/` | **唯一 HTTP 出口**（`src/api/http.ts` 的 **`adminRequest`** + 领域模块，如 **`admin-auth`**、**`content-admin-detail`**、**`content-transfer-records`**）；基址见 **`PUBLIC_ADMIN_API_ORIGIN`** 或开发同源代理（`apps/admin-web/rsbuild.config.ts` **`/admin`**）。 |
| `tests/` | rstest 测试与 `rstest.setup.ts`；新功能关键交互补测试。 |

### 2.3 React 实践

- 优先 **函数组件**；副作用集中在 `useEffect` 或与数据库无关的可预测位置。  
- 可组合逻辑用 **自定义 Hook**（`use*`），命名以领域含义为准。  
- **状态**：局部用 `useState`；跨多页共享再引入 Zustand/Redux 等（引入时在本节追加一行并说明边界）。  
- 若启用 **React Compiler**（`babel-plugin-react-compiler`），避免与编译器不兼容的写法；以构建配置为准。

### 2.4 样式

- 优先 **Tailwind 工具类**；全局样式放入有限入口（如 `App.css`），避免无序全局污染。  
- 设计令牌（颜色、间距）与 `packages/ui` 对齐时，以共享包导出为准，不在业务内复制魔法数。
