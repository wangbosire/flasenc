...
- **Schema**：`libs/database/prisma/schema.prisma`，**方言**：MySQL；连接串 **`DATABASE_URL`**（不落库，见 [`SECURITY.md`](SECURITY.md)）。**表结构说明与 ER 图**见 [`design-docs/content-sharing-platform-prisma-schema.md`](design-docs/content-sharing-platform-prisma-schema.md)。  
- **应用接入**：子应用 `imports: [DatabaseModule]`，注入 **`PrismaService`**（`@app/database`）；禁止在业务模块内 `new PrismaClient()`。  
- **本地与 E2E**：**日常开发不要求 Docker**；在 **本机安装 MySQL** 并创建库 **`flasenc`**，用 **`DATABASE_URL`**（见 `apps/server/.env.example`）指向即可。`jest.setup.ts` 与 `scripts/e2e-db-push.cjs` 在未设置 `DATABASE_URL` 时使用与 `.env.example` 一致的本地默认连接串。跑 **`test:e2e`** 前须保证库可达；`pretest:e2e` 会执行 **`prisma db push`**。仓库内 `docker-compose.mysql.yml` 与 `pnpm db:compose:*` **仅为可选备用**（无本机 MySQL 时自行选用），不作为默认流程。  
