...
- **本地与 E2E（标准路径：Docker）**：开发期间统一使用仓库根目录 **`compose.yml`**。应用容器固定命名为 **`flasenc`**，MySQL 容器为 **`flasenc-mysql`**，库名 **`flasenc`**，容器内连接串为 **`mysql://root:root123..@flasenc-mysql:3306/flasenc`**。执行 **`pnpm docker:dev`** 会安装依赖、生成 Prisma Client、应用迁移并 seed 本地管理账号。宿主机临时直连 Docker MySQL 时使用 **`127.0.0.1:3307`**。跑 **`test:e2e`** 前须保证 Docker 栈可达；`pretest:e2e` 会执行 **`prisma migrate deploy`**（应用 `libs/database/prisma/migrations/*`）。**日常改表**：进入 `flasenc` 容器后在 `apps/server` 执行 **`pnpm run db:migrate:dev`**；**对齐已有迁移文件**（空库 / 新环境 / 生产）：**`pnpm run db:migrate:deploy`**。若本地曾仅用 `db push` 建表且无 `_prisma_migrations`，须清空库或 `prisma migrate resolve` / `migrate reset` 后再对齐迁移记录（勿对含生产数据的库随意 reset）。
- **构建**：`prebuild` 执行 **`prisma generate`**（仅需 schema，不要求数据库在线）。
- **连通性自检**（Docker 栈已启动、`DATABASE_URL` 已配置或由 compose 注入时）：在 `apps/server` 执行 **`pnpm run db:test-connection`**（`Prisma` `$connect`）；结果会写入 **`.db-connection-test-result.txt`**（已加入 `.gitignore`）。
